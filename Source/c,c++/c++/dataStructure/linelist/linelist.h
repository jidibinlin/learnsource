...
status MakeNode(int e)//分配由p指向的值为e的节点，并返回ok;若分配失败返回error
    {
        p=new LNode;
        if(!p)
            return ERROR;
...
Position LocateElem(int e,status(*compare)(int,int))//返回线性链表L中第1个与e满足函数compare（）判定关系的元素的位置 若不存在这样的元素，则返回null
    {
        Link k=head;
        while(compare(e,k->data) && k!=tail)
            k=k->Next;
        return k;
    }
